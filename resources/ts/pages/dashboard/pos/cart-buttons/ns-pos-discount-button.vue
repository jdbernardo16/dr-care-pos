<template>
    <div @click="openDiscountPopup( order, 'cart' )" id="discount-button" class="flex-shrink-0 w-1/4 flex items-center font-bold cursor-pointer justify-center border-r flex-auto">
        <i class="mr-2 text-xl las la-percent"></i> 
        <span class="text-base">{{ __( 'Discount' ) }}</span>
    </div>
</template>
<script lang="ts">
