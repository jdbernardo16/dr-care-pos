    ...
    <div id="report-section" class="px-4">
        <div class="flex -mx-2">
            ...
            <div class="px-2">
                <button
                    @click="loadReport()"
                    class="rounded flex justify-between bg-input-background hover:bg-input-button-hover shadow py-1 items-center text-fontcolor px-2"
                >
            ...
            <div class="px-2">
                ...
                >
                    ...
                    <span class="pl-2">
                        {{ __("Category") }}:
                        {{ categoryNames || __("All Categories") }}
                    </span>
                </button>
            </div>
            <div class="px-2">
                ...
                >
                    <i class="las la-filter text-xl"></i>
                    <span class="pl-2">
                        {{ __("Unit") }}: {{ unitNames || __("All Units") }}
                    </span>
                </button>
            </div>
        </div>
                        ...
                        <table class="table ns-table w-full">
                            ...
                            <tbody>
                                <tr
                                    ...
                                    "
                                >
                                    <td class="p-2 border border-box-edge">
                                        {{ product.name }}
                                    </td>
                                    <td
                                        class="p-2 border text-right border-box-edge"
                                    >
                                        {{ product.unit_name }}
                                    </td>
                                    <td
                            ...
                            <tfoot class="font-semibold">
                                <tr>
                                    ...
                                    <td class="p-2 border text-right">
                                        {{ nsCurrency(totalPurchasePrice) }}
                                    </td>
                                    <td class="p-2 border text-right">
                                        {{ nsCurrency(totalSalePrice) }}
                                    </td>
                                    <td class="p-2 border text-right">
                                        {{ nsCurrency(totalTax) }}
                                    </td>
                                    <td class="p-2 border text-right">
                                        {{ nsCurrency(totalProfit) }}
                                    </td>
                                </tr>
                            </tfoot>
                        </table>
...
export default {
                ...
                return this.products
                    .map((product) => {
                        return math
                            ...
                            .subtract(
                                math
                                    .chain(product.total_purchase_price)
    ...
    methods: {
        ...
        async selectCategories() {
            try {
                const response = await selectApiEntities(
                    "/api/categories",
                    this.categoryField.label,
                ...
                if (error !== false) {
                    return nsSnackBar.error(
                        __(
                            "An error has occurred while loading the categories",
                        ),
                    );
                }
            }
        ...
        async selectUnit() {
            try {
                ...
                if (error !== false) {
                    return nsSnackBar.error(
                        __("An error has occurred while loading the units"),
                    );
                }
            }
        },
        ...
        loadReport() {
            ...
            nsHttpClient
                .post("/api/reports/profit-report", {
