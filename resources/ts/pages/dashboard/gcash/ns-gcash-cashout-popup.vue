    ...
    <div>
        <div class="shadow-lg w-[95vw] md:w-132 ns-box">
            <div class="border-b ns-box-header p-2 text-fontcolor flex justify-between items-center">
                <h3 class="font-semibold">{{ __( 'GCash Cash Out' ) }}</h3>
                <div><ns-close-button @click="popup.close()"></ns-close-button></div>
            </div>
            <div class="p-4">
                <div class="mb-4">
                    <label class="block text-sm font-semibold text-secondary mb-1">{{ __( 'Cash Amount Requested' ) }}</label>
                    <input
                        v-model="amount"
                        type="number"
                        step="1"
                        ...
                        @input="computeFee"
                    />
                    <p class="text-xs text-secondary mt-1">{{ __( 'The amount of cash the customer wants to receive.' ) }}</p>
                </div>

                <div v-if="amount > 0" class="mb-4 p-3 rounded-lg bg-surface border border-edge space-y-2">
                    <div class="flex justify-between text-sm">
                        <span>{{ __( 'Cash Given to Customer' ) }}</span>
                        <span class="font-semibold">{{ nsCurrency( Number( amount ) ) }}</span>
                    </div>
                    <div class="flex justify-between text-sm">
                        <span>{{ __( 'Service Fee (stays in GCash)' ) }}</span>
                        <span class="font-semibold">+ {{ nsCurrency( computedFee ) }}</span>
                    </div>
                    <div class="border-t border-edge pt-2 flex justify-between font-bold">
                        <span>{{ __( 'Customer Sends via GCash' ) }}</span>
                ...
                <div class="flex justify-end gap-2">
                    ...
                    <div class="ns-button warning"><button class="px-4 py-2" @click="submit" :disabled="submitting || !amount">{{ submitting ? __( 'Processing...' ) : __( 'Confirm Cash Out' ) }}</button></div>
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        submit() {
            ...
            nsHttpClient.post( `/api/gcash/cash-out/${this.popup.params.session.id}`, {
                amount: this.amount,
                customer_phone: this.customerPhone || null,
                reference_no: this.referenceNo || null,
            }).subscribe({
                next: ( result ) => {
                    nsSnackBar.success( result.message || __( 'Cash-out processed.' ) );
                    this.popup.params.resolve?.();
                    this.popup.close();
                },
                error: ( err ) => {
                    nsSnackBar.error( err.message || __( 'Failed to process cash-out.' ) );
