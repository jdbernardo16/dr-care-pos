        ...
        <div id="database-update" class="w-full md:w-2/3 lg:w-1/3">
            ...
            <div class="my-3 rounded shadow ns-box">
                ...
                <div class="p-2 ns-box-body">
                    ...
                    <div
                        v-if="error"
                        class="border-l-4 text-sm ns-notice error p-4"
                    >
                        <p>
                            {{
                                __(
                        ...
                        <pre class="rounded whitespace-pre-wrap my-2 p-2">{{
                            lastErrorMessage
                        }}</pre>
                    </div>
                </div>
                <div class="border-t ns-box-footer p-2 flex justify-between">
                    ...
                    <div class="flex">
                        ...
                        >
                            ...
                            <span v-if="!updatingModule"
                                >{{ __("Updating") }}...</span
                            >
                            <span class="mr-1" v-if="!updatingModule"
                                >{{ index }}/{{ files.length }}</span
                            >
                            <span v-if="updatingModule"
                                >{{ __("Updating Modules") }}...</span
                            >
                            <span class="mr-1" v-if="updatingModule"
                                >{{ index }}/{{ totalModules }}</span
                            >
                        </ns-button>
                        <ns-button
                            type="info"
...
export default {
    ...
    mounted() {
        nsHttpClient.get("/sanctum/csrf-cookie").subscribe((_) => {
            try {
                this.xXsrfToken =
                    nsHttpClient.response.config.headers["X-XSRF-TOKEN"];
                this.proceedUpdate();
            } catch (e) {
            ...
            for (let index in this.files) {
                try {
                    ...
                    this.lastErrorMessage =
                        exception.message || __("An unexpected error occurred");
